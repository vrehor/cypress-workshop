describe('Update card api test', {env: {hideCredentials: true}}, async () => {
  before(() => {
        ...
        if (data?.createCard.__typename === 'Card') {
          cy.updateCard({
            ...
            if (data?.updateCard.__typename === 'Card') {
              ...
              expect(data?.updateCard.deadline).to.be.eq(testDeadline.toISOString());
              expect(data?.updateCard.description).to.be.eq("test description");
