...
export const NewBoardListCard = ({board}: Props) => {
  ...
  return <S.Wrapper>
    {!showForm
      ? <S.Title onClick={() => setShowForm(true)}>Create new list</S.Title>
      : <S.Form form={form} style={{width: '100%'}}
        ...
        >
          <Input prefix={<UserOutlined className="site-form-item-icon"/>} placeholder="List name"/>
        </Form.Item>
        <Form.Item>
          <Button type="primary" htmlType="submit" block className="save-form-button">
            Save
          </Button>
          <Button type={'text'} block onClick={onCancel}>cancel</Button>
        </Form.Item>
      </S.Form>}
  </S.Wrapper>
}
