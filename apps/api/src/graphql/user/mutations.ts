...
export const login = mutationField('login', {
  ...
  async resolve(
    ...
    if (user && (await HashService.compareString(password, user.password))) {
      ...
      return addTypeName(await UserDao.update(user, {id: user.id}), 'LoginData')
    }

    return addTypeName({message: 'User is not authorizes. Check combination of email and password'}, 'AuthorizationError')
  }
});
